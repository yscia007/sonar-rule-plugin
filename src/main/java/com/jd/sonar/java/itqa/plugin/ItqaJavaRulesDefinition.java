 ...
 * @desc: description
 */
public class ItqaJavaRulesDefinition implements RulesDefinition {

    // don't change that because the path is hard coded in CheckVerifier
    private static final String RESOURCE_BASE_PATH = "/com/jd/sonar/java/itqa/rules";

    public static final String REPOSITORY_KEY = "JD-itqa-java";

    private final Gson gson = new Gson();

    private static final Set<String> TEMPLATE_RULE_KEY = ImmutableSet.of("QA113");

    @Override
    public void define(Context context) {
        NewRepository repository = context
                .createRepository(REPOSITORY_KEY, Java.KEY)
                .setName("JD itqa Repository");
    ...
    private void addMetadata(NewRule rule, String metadataKey) {
        URL resource = ItqaJavaRulesDefinition.class.getResource(RESOURCE_BASE_PATH + "/" + metadataKey + "_java.json");
        if (resource != null) {
    ...
    private static void addHtmlDescription(NewRule rule, String metadataKey) {
        URL resource = ItqaJavaRulesDefinition.class.getResource(RESOURCE_BASE_PATH + "/" + metadataKey + "_java.html");
        if (resource != null) {
